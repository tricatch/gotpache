...
public class CmdMonitor implements ConsoleCommand {

    private static final Logger logger = LoggerFactory.getLogger(CmdMonitor.class);
    ...
    public ConsoleResponse execute(String uri, Map<String, String> params) {
        ...
        try {
            ...
            String content = FreeMarkerUtil.renderTemplate("monitor.ftl", dataModel);
            return ConsoleResponseBuilder.ok(content);
        } catch (IOException | TemplateException e) {
            logger.error("Failed to render monitor page", e);
            try {
